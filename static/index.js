$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
          var tr =
            '<tr></tr><td style="text-align: left"><a href="/static/' +
            data[i].file_path +
            ...
            data[i].update_time +
            "</td>\n" +
            '<td><a class="btn btn-success" href="/download_file/?file_path=' +
            data[i].file_path +
            '"><i class="fa fa-cloud-download fa-lg" aria-hidden="true"></i> 下载</a>' +
            ...
            '<button class="btn btn-warning rename_file" data-toggle="modal" data-target="#FileModal' +
            data[i].id +
            '"><i class="fa fa-gear fa-lg" aria-hidden="true"></i> 重命名</button>\n' +
            '<button class="btn btn-danger"><span href="javascript:void(0)" class="deleteFile" title="' +
            data[i].file_path +
            '"><i class="fa fa-trash fa-lg" aria-hidden="true"></i>删除</span></button>\n' +
            "</td></tr>";
          all_tr = all_tr + tr;
        }
  ...
  $(".shareFile").click(function () {
    // alert("?");
    if (event.target.className == "fa fa-share-alt") {
      var file_path = event.target.parentElement.title;
    } else {
      var file_path = event.target.title;
    }
    ...
    // var file_path = $(".shareFile").title;
    var file_sharecode = document.getElementsByName("file_sharecode")[0].value;
    alert(file_sharecode);
    var formData = new FormData();
    formData.append("file_path", file_path);
